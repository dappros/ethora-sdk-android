...
## [26.05.15] — JitPack `v1.0.40`

- **Fixed:** Host-side unread preview surfaces (an inbox screen, a separate "what's new" tab, anything reading `RoomStore.rooms` outside the `Chat` composable) now keep showing real unread state after the user tab-swaps through the chat. The `v1.0.39` dispose / visibility / `onChatPaused` paths all wrote `lastViewedTimestamp = System.currentTimeMillis()`, which marked every existing message as "read" even when the user merely flicked through the room without scrolling — the host's filter `ts > lastViewedTimestamp` then returned an empty list. The three call sites (`ChatRoomView.DisposableEffect.onDispose`, `ChatRoomView` active-viewing `snapshotFlow` collector, and `ChatService.lifecycle.onChatPaused`) now resolve the marker via a shared `RoomStore.resolveReadMarkerOnLeave(roomJid)` helper: it advances the marker to the latest message the SDK actually knows about for that room (`Room.lastMessageTimestamp ?? MessageStore.lastKnownTimestamp(jid)`), or keeps the existing marker if no progress was recorded. Typical chat UX (badge clears when the user leaves a room they read) is unchanged; passive nav-through preserves baseline so host previews stay accurate.
- **Fixed:** `EthoraChatBootstrap` waited only 8 seconds for the WebSocket TLS+SASL+bind handshake before declaring the run failed and dropping into a 15-second cooldown — too tight for AWS lambda cold-starts, congested networks, or first connect after the server has been idle. Bootstrap aborted before `InitBeforeLoadFlow.run` / history preload / catchup ever fired, so `MessageStore` stayed empty for the whole session and host apps reading from it (badge listeners, host-side unread previews) saw nothing. Timeout raised to 30 s — comfortably above the observed P99 connect time, well under any "this app is dead" UX threshold. The retry path is unchanged.
- **Build:** `gradle.properties` `VERSION_NAME` bumped to `1.0.40` (was stuck at `1.0.0`).
